...
window.onload=function(){
	...
	var span2Text=document.getElementById("span2");
	//确认密码
	var rpw=document.getElementById('repassword');
	var span3Text=document.getElementById("span3");
	//邮箱
	var em=document.getElementById('email');
	var span4Text=document.getElementById("span4");
	//手机号码
	var ph=document.getElementById('phone');
	var span5Text=document.getElementById("span5");
	// 手机号码获取焦点，设置提示信息
    em.onfocus = function () {
        // 设置提示信息span的类名和信息
        span5Text.className = 'tipMsg';
        span5Text.innerHTML = '请输入正确格式的邮箱';
    }
	// 邮箱获取焦点，设置提示信息
    ph.onfocus = function () {
        // 设置提示信息span的类名和信息
        span5Text.className = 'tipMsg';
        span5Text.innerHTML = '正确的手机号码是你收货的保障';
    }
    ph.onblur=function(){
    	var pho=ph.value;
 		// 获得文件框值:去除多余的空白字符
 		var phone = pho.trim();
 		 var phonereg= /^1(3|4|5|7|8)\d{9}$/;    //手机号码格式
          if(!phonereg.test(pho)){
            	 span5Text.className = 'error';
              	span5Text.innerHTML = '手机号码格式不正确';
            	 }else{
            		 span5Text.className = 'success';
                   	span5Text.innerHTML = '√'; 
            	 }
    }
    em.onblur=function(){
		var ema=em.value;
 		// 获得文件框值:去除多余的空白字符
 		var email = ema.trim();
 		 var emreg= /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;    //邮箱格式
         // 判断是否为空
         if (email == '') {
         	span4Text.className = 'error';
         	span4Text.innerHTML = '邮箱不允许为空';
             // 终止程序
             return;
             }else if(!emreg.test(ema)){
            	 span4Text.className = 'error';
              	span4Text.innerHTML = '邮箱格式不正确';
            	 }else{
            		 span4Text.className = 'success';
                   	span4Text.innerHTML = '√'; 
            	 }
       }
	// 确认密码获取焦点，设置提示信息
    rpw.onfocus = function () {
        // 设置提示信息span的类名和信息
        span3Text.className = 'tipMsg';
        span3Text.innerHTML = '请输入与上一致的密码';
    }
    rpw.onblur=function(){
    	 if (rpw.value.trim() == '') {
          	span3Text.className = 'error';
          	span3Text.innerHTML = '密码不允许为空';
              // 终止程序
              return;
            }
    	 else if(pw.value!=rpw.value){
    		 span3Text.className = 'erro';
    	     span3Text.innerHTML = '密码不一致';
    	}else{
    		span3Text.className = 'success';
   	     span3Text.innerHTML = '√';
    	}
    }
	// 密码获取焦点，设置提示信息
    pw.onfocus = function () {
        // 设置提示信息span的类名和信息
        span2Text.className = 'tipMsg';
        span2Text.innerHTML = '为了您的账户安全密码需8到12个字符';
    }
    pw.onblur=function(){
    	/* // 判断是否有非法字符(除了中英文、数字、下划线以外的字符)
        var charReg = /[^\u4E00-\u9FA5\w]/;
	  //纯数字
		...
		var pword=pw.value;
 		// 获得文件框值:去除多余的空白字符
 		var password = pword.trim();
 		 var pwreg= /^[0-9]{6,16}$|^[a-zA-Z]{6,16}$/;    //全是数字或全是字母     6-16个字符
         var pwreg1= /^[A-Za-z0-9]{6,16}$/;     //数字、26个英文字母      6-16个字符
         var pwreg2= /^\w{6,16}$/;           // 由数字、26个英文字母或者下划线组成的字符串    6-16个字符 
         // 判断是否为空
         if (password == '') {
         	span2Text.className = 'error';
         	span2Text.innerHTML = '密码不允许为空';
             // 终止程序
             return;
             }else if(charReg.test(pword)){
                 // 如果res为真，表示有非法字符
                     // 设置错误信息
                     span2Text.className = 'error';
                     span2Text.innerHTML = '密码仅支持中英文、数字和下划线且不能存在非法字符';
                     // 终止程序
                     return;
             } else if(numReg.test(password)){
            	// 设置错误信息
                 span2Text.className = 'error';
                 span2Text.innerHTML = '密码仅支持中英文、数字和下划线且不能为纯数字';
                 // 终止程序
                 return;
             } // 判断是否满足条件
                 ...
                 // 设置错误信息
                 span2Text.className = 'error';
                 span2Text.innerHTML = '密码不能少于8个字符或多于12个字符';
                 // 终止程序
                 return;
             }  
             	...
             	// 设置错误信息
                  span2Text.className = 'error';
                  span2Text.innerHTML = '弱';
                  // 终止程序
                  return;
              } 
             else if(pwreg1.test(password)){
             	// 设置错误信息
                  span2Text.className = 'weak';
                  span2Text.innerHTML = '中';
                  // 终止程序
                  return;
              }
             else if(pwreg2.test(password)){
             	// 设置错误信息
                  span2Text.className = 'success';
                  span2Text.innerHTML = '强';
                  // 终止程序
                  return;
              }
    }     
     ...
     un.onfocus = function () {
         // 设置提示信息span的类名和信息
         spanText.className = 'tipMsg';
         spanText.innerHTML = '设置后不可更改,5到8个字符';
     }
     un.onblur=function(){
 		// 获得文件框值:去除多余的空白字符
 		var uname=un.value;
         ...
         if (username == '') {
         	...
         	spanText.innerHTML = '用户名不允许为空';
             // 终止程序
             return;
             } else if(charReg.test(uname)){
                 // 如果res为真，表示有非法字符
                     // 设置错误信息
                     spanText.className = 'error';
                     spanText.innerHTML = '用户名仅支持中英文、数字和下划线且不能存在非法字符';
                     // 终止程序
                     return;
             } else if(numReg.test(uname)){
            	// 设置错误信息
                 spanText.className = 'error';
                 spanText.innerHTML = '用户名仅支持中英文、数字和下划线且不能为纯数字';
                 // 终止程序
                 return;
             }
         // 判断是否满足条件
         else if ((uname.length)<5||(uname.length)>8) {
             // 设置错误信息
             spanText.className = 'error';
             spanText.innerHTML = '用户名不能少于5个字符或多于8个字符';
             // 终止程序
             return;
         }
         		else{
         		...
         		xhr.onreadystatechange = function(){
         			if(xhr.readyState == 4){
         				if(xhr.status == 200){
         					spanText.innerHTML = xhr.responseText;
         				}
         			}
         		}
         		// 3.打开连接
						...
						<table>
							<tbody><tr>
								...
								<td>
									...
									<span id="span1" class="tipMsg"></span>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									...
									<span id="span2" class="tipMsg"></span>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									...
									<span id="span3" class="tipMsg"></span>
								</td>
							</tr>
							<tr>
								<th>
								...
								<td>
									...
									<span id="span4" class="tipMsg"></span>
								</td>
							</tr>
									<tr>
										<th>
									...
									<tr>
										...
										<td>
												...
												<span id="span5" class="tipMsg"></span>
										</td>
									</tr>
